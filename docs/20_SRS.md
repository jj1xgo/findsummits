...
- **外部サービス**
  - CDN（Leaflet・SheetJS・JSZip。HTML ビューアがブラウザから実行時取得。[9 参照](#9-外部システム依存関係環境)）
...
- **概要**: 国土数値情報の市区町村単位の行政区域 GeoJSON（約 12 万フィーチャ（市区町村単位の飛び地を含む））から、後段 FR が利用する 4 種類の派生データを生成する初回限定のデータ準備処理。なぜ前処理が必要か:
  ...
  - 北方領土（[ADR-URD-005](decisions/ADR-URD-005-northern-territories-exclusion.md) 参照）はタイル取得段階（[FR-001](#fr-001-標高タイル事前取得)）で除外したいが、解析エンジンに GIS ライブラリを持ち込まずに済むよう、除外対象タイルのリストを事前生成する
...
- **実行タイミング**: 初回のみ。[7.2.3](#723-n03-行政区域-zip) に N03 ZIP が配置され、かつ対応する出力ファイル群が未生成の場合に起動する。N03 ZIP を新年版に差し替えた場合は出力ファイル群を削除してから再実行することで新しい ZIP を反映できる（削除・再実行の操作詳細は HLD 参照）。日本全土1次メッシュコードリストを更新した場合も、出力ファイル群を削除してから再実行することで北方領土除外メッシュリストへ反映できる。生成時に採用した N03 ZIP の版（YYYYMMDD）を出力ファイル群と対応づけて記録し（記録形式は HLD）、出力ファイル群が既にあって生成を省く場合も、配置済み ZIP の最新の版が記録した版より新しければ、削除・再実行を促す警告ログを出す
...
- **フォールバック（後段 FR の動作）**:
  - N03 前処理済み地域 GeoJSON が未生成の場合: [FR-009](#fr-009-sotaリスト突合match_status-判定) は SOTA エリアコード判定をスキップし、地域不明を示す仮サミットコード（`ZZ/ZZ-A00` 形式）を付与する（警告ログ出力。詳細: [FR-009 参照](#fr-009-sotaリスト突合match_status-判定)）
  - N03 前処理済み市区町村 GeoJSON が未生成の場合: [FR-009](#fr-009-sotaリスト突合match_status-判定) は市区町村名を空文字として続行する（警告ログ出力）
  - 北方領土除外タイルリストが未生成の場合は、それを読む [FR-001](#fr-001-標高タイル事前取得) がエラー終了する。北方領土除外メッシュリストが未生成の場合は、それを読む [FR-001](#fr-001-標高タイル事前取得)・[FR-004](#fr-004-33メッシュ結合解析オーケストレーション)・[FR-008](#fr-008-per-mesh-csv-統合)・[FR-014](#fr-014-広域結合解析オーケストレーション) がエラー終了する（読む FR は [8.1](#81-内部データ一覧) No.3・No.16）。出力1・2の未生成が成果物上で識別可能な劣化（仮サミットコード・空文字＋警告ログ）に留まるのに対し、北方領土除外は本リストによるタイル取得スキップ・除外メッシュ扱いのみで担保され後段に除外手段がない（生成1・2参照）ため、未生成のまま続行すると管轄外のサミット候補が検出不能な形で成果物へ混入する。これを防ぐ安全側の設計としてエラー終了とする（詳細: [ADR-SRS-018](decisions/ADR-SRS-018-northern-territories-skip-at-tile-fetch.md)・[ADR-SRS-053](decisions/ADR-SRS-053-northern-territories-tile-intersection-and-excluded-meshes.md) 参照）
...
**説明**:
  ...
  - **取得範囲**: メッシュコードリストの各メッシュコードについて、そのメッシュをカバーする最小範囲の標高タイルを取得する（隣接メッシュへの拡張は行わない。処理範囲175メッシュを一括取得する運用では、メッシュ境界付近のタイルが複数メッシュから重複して取得されるが問題ない）
  ...
  - タイルの取得成功時（HTTP 200）は、レスポンスヘッダー `Last-Modified` の日時をキャッシュ PNG ファイルの mtime に設定する。これにより mtime が「ダウンロード日時」ではなく「提供元更新日」を正確に表す（[ADR-SRS-032](decisions/ADR-SRS-032-gsi-tile-latest-date-provenance.md)）。地理院タイルサーバーは静的 PNG 配信のため HTTP 200 時に必ず `Last-Modified` を返すことを確認済みとする
  - サーバーがリクエスト過多・一時利用不可を示す応答を返した場合は再試行する（該当する応答の具体値は HLD で定義する）。リトライ上限を超えた場合はエラーを出力して処理を中断する（タイルが欠損したまま解析を継続すると NODATA 増加で解析品質が静かに低下するため、ユーザーに再実行を促す方針を優先する）。リトライ上限回数・バックオフ戦略の詳細は HLD で定義する
  ...
  - 次のタイルのローカルキャッシュが存在する場合は削除する。中心点判定など旧方式で取得済みのタイルが残ったまま解析に入らないようにするため: ①北方領土除外タイルリストのタイル（DEM10b は上記「北方領土除外」と同じく Z14 へ変換したタイル）、②北方領土除外メッシュの取得範囲のタイルのうち、日本全土1次メッシュコードリストの除外メッシュでないメッシュの取得範囲にも含まれないもの。②は今回の 1次メッシュコードリストで範囲を絞った場合も全土基準で判定し、範囲外の隣接メッシュ用に取得済みの境界タイルを消さない
  - **異常系**: 北方領土除外タイルリストまたは北方領土除外メッシュリストが存在しない場合はエラー終了する（詳細は上記「北方領土除外」参照）。リクエスト過多・一時利用不可の応答に対するリトライ上限を超えた場合もエラーを出力して処理を中断する（詳細は上記参照）。それ以外の取得失敗（未整備を示す応答以外のエラー応答、タイムアウト、通信断）も、欠損タイルを残したまま正常終了せず、エラーを出力して処理を中断する（再試行の有無・上限は HLD で定義する）

---

### 4.3 パイプライン制御コンポーネント

フェーズ2〜4 の実行順序と N エスカレーションループを統括する制御コンポーネント（実装方式は [ADR-SRS-027](decisions/ADR-SRS-027-fr022-purification-fr023-pipeline-control.md) と HLD）。
...
**説明**:
  ...
  - **実行順序（フェーズ2〜4 の制御）**:
    1. **フェーズ2（3×3 解析）**: [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) を全メッシュに対して実行し（北方領土除外メッシュは [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) が省略する）、per-mesh CSV・per-mesh ピーク候補 GeoJSON・標高地形図を生成する
  ...
  - **各段の独立起動**: フェーズ2 / フェーズ3（各 N）/ フェーズ4 はそれぞれ単独で起動できる（フェーズ指定の起動オプションを提供する。具体的な構文は HLD で定義）。各段で中間成果物の目視確認が可能（自動ループは各段の動作が確認できてから利用することを推奨）。起動前提条件: フェーズ2 単独起動には [FR-001](#fr-001-標高タイル事前取得) による標高タイルのローカルキャッシュ取得完了が必要。フェーズ3 単独起動には `merged_peak.csv` と `key_col_unresolved_peaks-<前段N>.csv` の存在が必要。フェーズ4 単独起動には `merged_peak.csv`・`merged_peak.geojson` と標高タイルのローカルキャッシュ（[FR-009](#fr-009-sotaリスト突合match_status-判定) が `gsi_tile_latest_date` の算出に使う）の存在が必要。起動前提条件の充足検査は本 FR が行い、不足時はエラー終了する（フェイルファースト）。エラーメッセージの詳細は HLD で定義する
...
| 解析済みウィンドウメッシュ集合 | 内部データ | — | ウィンドウ内の存在メッシュコード（北方領土除外メッシュを除く）のソート済みリスト。ファイル名 `<解析識別子>.meshset`（物理形式は HLD 定義）。[FR-014](#fr-014-広域結合解析オーケストレーション) の無効パターンスキップ判定が読み取る（[ADR-SRS-046](decisions/ADR-SRS-046-analyzed-window-meshset-skip.md)） |

**説明**:

  - 1次メッシュコードリストが省略された場合は、日本全土1次メッシュコードリストから北方領土除外メッシュリストのメッシュを除いた集合を解析対象として使用する
  - 入力された 1次メッシュコードリストの各メッシュコードが「日本全土1次メッシュコードリスト」に含まれていない場合、警告ログを出力してそのコードをスキップし、残りのメッシュコードについて処理を継続する
  - 入力された 1次メッシュコードリストの各メッシュコードが北方領土除外メッシュリストに含まれる場合も、同様に警告ログを出力してそのコードをスキップする（明示指定された場合の扱い。省略時の既定は上記のとおり最初から除いている）
  - **対象メッシュ（処理対象として順に選ぶメッシュ）は北方領土除外メッシュにならない**: 上記のとおり、1次メッシュコードリスト（省略時の既定・明示指定のどちらも）から北方領土除外メッシュが除かれた集合のみを対象メッシュとして順に解析する。したがって対象メッシュを北方領土除外メッシュ自身が中心になって解析されることはなく、per-mesh CSV・GeoJSON・標高地形図・解析識別子も生成されない。日本全土1次メッシュコードリストに含まれる北方領土除外メッシュそれぞれについて、対象メッシュとして選ばれず省略したことを実行ログに記録する（詳細: [ADR-SRS-053](decisions/ADR-SRS-053-northern-territories-tile-intersection-and-excluded-meshes.md)）
  ...
  - **範囲を絞った実行の位置づけ**: 1次メッシュコードリストで範囲を絞った実行は、動作確認・試験用とする。リスト外となった隣接の陸地メッシュは NODATA 充填されるため、全国実行と結果が異なりうる（尾根が断たれて海面確定規則が誤って働く場合を含む）。申請用の結果は、1次メッシュコードリストを省略した全国実行から得る（[NFR-001](#nfr-001-精度プロミネンス判定) の測定地点と同じ）
  - メッシュコードリストをコード昇順にソートしてから、1 メッシュずつ順番に解析する（処理順序の再現性確保のため。順序は解析結果に影響しない）
  - **解析対象メッシュの決定**: 対象メッシュ（中心。北方領土除外メッシュではない）を中心とした 3×3 のメッシュグリッド（最大 9 メッシュ）のうち、1次メッシュコードリスト（省略時は日本全土1次メッシュコードリスト）に存在し、かつ北方領土除外メッシュリストに含まれないものを解析対象メッシュとする（除外メッシュはリスト外メッシュと同じ扱いとなり、解析対象メッシュ範囲情報・解析済みウィンドウメッシュ集合からも外れる。窓の bounding box の決定には用いないが、除外メッシュの地理的範囲が bbox 内に位置する場合はその範囲を NODATA 充填する。詳細: [ADR-SRS-053](decisions/ADR-SRS-053-northern-territories-tile-intersection-and-excluded-meshes.md)）。
...
**異常系**: 解析対象メッシュ（1次メッシュコードリスト内で3×3グリッドに含まれ、かつ北方領土除外メッシュリストに含まれないメッシュ）のローカルキャッシュタイルが 1 枚も存在しない場合、エラーを出力して処理を中止する（タイル欠如のまま解析を続行すると全ピクセルが NODATA となり、海面確定規則が誤発動して存在しないコルが確定する恐れがある）。タイル事前取得（[FR-001](#fr-001-標高タイル事前取得)）の未実行・取得漏れが疑われるため、原因の確認と再実行をユーザーに促す。この停止は地理院側未整備によるタイル全欠如も対象とし、意図的除外（北方領土除外メッシュ）とは区別する（詳細: [ADR-SRS-053](decisions/ADR-SRS-053-northern-territories-tile-intersection-and-excluded-meshes.md)）。NODATA 充填で続行が許されるのは「1次メッシュコードリスト外のメッシュ（海域・国外）」「北方領土除外メッシュ」と「存在メッシュ内の部分的欠損（地理院未整備等）」のみ
...
| 解析済みウィンドウメッシュ集合 | 内部データ | — | ウィンドウ内の存在メッシュコード（北方領土除外メッシュを除く）のソート済みリスト。ファイル名 `<解析識別子>.meshset`（物理形式は HLD 定義）。各コーナーパターン解析完了後に書き出す（[ADR-SRS-046](decisions/ADR-SRS-046-analyzed-window-meshset-skip.md)） |
...
**説明**:
  ...
  - **使用タイルズームレベル**: ズームレベル14。タイル結合時に、フォールバック・デコード後の標高を 2×2 ピクセル max pooling でズームレベル15 → 14 にダウンサンプリングする（別途タイル取得は不要。順序は下記「解析範囲標高グリッドの生成」）
  - **ピーク座標は L15 座標を維持する**: max pooling では 2×2 ピクセルのうち採用した最大値ピクセルのズームレベル15 座標を保持し、ピークの出力座標はズームレベル15 相当の座標とする（pooling は標高値の縮約であり、座標まで縮約しない）。2×2 に同じ最大値の画素が複数あるときは、北の行を先に、同じ行は西を先に見て最初の画素の座標を保持する（[FR-005](#fr-005-ピーク候補検出) のタイブレークと同じ向き。DEM10b だけの地点では 2×2 が同値になるため、通常解析がピークとする画素と一致させる。[ADR-SRS-055](decisions/ADR-SRS-055-wide-analysis-pooling-after-fallback.md)）。これにより [FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) の対象ピーク絞り込みと [FR-008](#fr-008-per-mesh-csv-統合) の重複排除（ズームレベル15 のピクセル座標（タイル座標とタイル内の画素位置の組）の一致判定）が通常 per-mesh と同一基準で機能する
  - **4 コーナーパターン生成**: 対象メッシュを N×N グリッドの **4 つのコーナー位置**に置いた **4 通り** の N×N メッシュコードリストを内部生成する。コーナーは対象メッシュを置く位置を表し、ウィンドウは対角方向に展開する:
    ...
    - SW: 南西隅に配置（北・東方向を探索）
    - SE: 南東隅に配置（北・西方向を探索）
  - **コーナー配置を採る理由**: 対象ピークから対角方向に最大 (N-1)√2 メッシュ離れたコル候補までカバーできる（同じ対角方向で比べると、中央配置の到達距離 (N-1)√2/2 の 2 倍）。4 コーナー（NW/NE/SW/SE）で全方位を覆う
  - **無効パターンのスキップ**: ウィンドウ内のメッシュのうち日本全土1次メッシュコードリストに存在しないもの（海域・国外等）、および北方領土除外メッシュリストに含まれるものは、タイル未取得領域として NODATA 充填し解析を続行する（[FR-004](#fr-004-33メッシュ結合解析オーケストレーション) のリスト外メッシュ・除外メッシュと同じ扱い。詳細: [ADR-SRS-053](decisions/ADR-SRS-053-northern-territories-tile-intersection-and-excluded-meshes.md)）。各コーナー試行前に、候補ウィンドウの存在メッシュコード集合（北方領土除外メッシュを除く）を算出し、蓄積済みの全 `.meshset` ファイルのうち**いずれか 1 件**に候補集合が包含されていれば（部分集合判定）、そのパターンは試行せずスキップする（解析しても新たな情報が得られないため）。複数ファイルの和集合との比較ではなく、単一ファイルとの包含関係で判定する（[ADR-SRS-046](decisions/ADR-SRS-046-analyzed-window-meshset-skip.md) 参照）。N=5 で N=4 の配置と完全に重複するケースもこの判定で自然にスキップされる
  - **解析範囲標高グリッドの生成**: コーナー配置パターンごとに、[FR-004](#fr-004-33メッシュ結合解析オーケストレーション) と同様の手順（解析対象メッシュ全体を囲む最小の矩形（bounding box）の決定・タイル結合・[FR-002](#fr-002-dem-階層フォールバック)/[FR-003](#fr-003-標高デコードnodata-処理) 適用・1px 海面ボーダー付加）で解析範囲標高グリッドを生成する（ズームレベル14）。順序は、まずズームレベル15 の各画素で [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) と同じく [FR-002](#fr-002-dem-階層フォールバック) のフォールバックと [FR-003](#fr-003-標高デコードnodata-処理) のデコードを行い（DEM10b は Z15 の 2×2 へ最近傍で対応付ける）、得た標高を 2×2 max pooling でズームレベル14 へ縮約する。NODATA（-9999m）は有効値に負けるため、4 画素すべてが NODATA のときだけ NODATA になる。処理はタイル単位で行い、ズームレベル15 の結合画像全体は作らない（[ADR-SRS-055](decisions/ADR-SRS-055-wide-analysis-pooling-after-fallback.md)）
...
- 対象メッシュを中心とした 3×3 グリッド（フェーズ2 = [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) が解析した範囲と同一）の存在メッシュ（日本全土1次メッシュコードリストに存在し、かつ北方領土除外メッシュリストに含まれないメッシュ。次項と同じ定義）にローカルキャッシュタイルが 1 枚も存在しない場合、エラーを出力して処理を中止する（タイル事前取得（[FR-001](#fr-001-標高タイル事前取得)）の未実行・取得漏れが疑われるため、原因の確認と再実行をユーザーに促す）
- ウィンドウ（bounding box）内の存在メッシュ（日本全土1次メッシュコードリストに存在し、かつ北方領土除外メッシュリストに含まれないメッシュ）にローカルタイルが 1 枚も存在しない場合も、エラーを出力して処理を中止する（該当メッシュコードを表示し、prefetch 再実行をユーザーに促す。タイル欠如のまま解析を続行すると全ピクセルが NODATA となり、海面確定規則が誤発動して存在しないコルが確定する恐れがある）。この停止は地理院側未整備によるタイル全欠如も対象とし、意図的除外（北方領土除外メッシュ）とは区別する（詳細: [ADR-SRS-053](decisions/ADR-SRS-053-northern-territories-tile-intersection-and-excluded-meshes.md)）。NODATA 充填で続行が許されるのは「リスト外メッシュ（本物の海・国外）」「北方領土除外メッシュ」と「存在メッシュ内の部分的欠損（地理院未整備等）」のみ
...
- **対応 UR**: [UR-001](10_URD.md#ur-001), [UR-002](10_URD.md#ur-002), [UR-005](10_URD.md#ur-005)
...
| analysis_id | str | — | 解析識別子。通常モード: `3-<中心メッシュコード>`（例: `3-5239`）、広域モード: `<N>-<対象メッシュコード>-<コーナー>`（例: `4-5239-NW`）。mode・meshcode・corner・zoom の全情報を内包する（[ADR-SRS-021](decisions/ADR-SRS-021-per-mesh-csv-column-design.md) 参照） |
  ...
  - **広域モード（フェーズ3・[FR-014](#fr-014-広域結合解析オーケストレーション) 呼び出し時）の出力動作**:
    - **対象ピーク絞り込み**: 解析範囲内で検出された全ピークではなく、[FR-014](#fr-014-広域結合解析オーケストレーション) から渡された対象ピーク座標リストに含まれるピークの行のみを per-mesh CSV に出力する（merged 統合時のノイズを防ぐため）。一致判定は対象ピーク座標リストの緯度経度を解析に使用したズームレベル15 のピクセル座標（タイル座標とタイル内の画素位置の組）へ変換し、検出ピークの同座標と比較する（[FR-008](#fr-008-per-mesh-csv-統合) の重複排除と同一規則）
...
- **対応 UR**: [UR-002](10_URD.md#ur-002), [UR-003](10_URD.md#ur-003)
...
| 陸地最高峰リスト | 内部データ | 必須 | — | [8.1 参照](#81-内部データ一覧)。富士山・旭岳・中岳（九重）の緯度経度リスト。`merged_peak.csv` 生成時に陸地最高峰の海面確定（`key_col_resolved=true`・Key コル = 0m）を適用する |
...
| 日本全土1次メッシュコードリスト | 内部データ | 必須 | — | `expected_count` 算出の基準母集合（北方領土除外メッシュを除く。[8.1 参照](#81-内部データ一覧)）。`対象1次メッシュコードリスト` で範囲を絞っても本リストは常に全土基準のまま使用する |
| 北方領土除外メッシュリスト | 内部データ | 必須 | — | [FR-017](#fr-017-n03-行政区域前処理データ準備) が生成。`expected_count` の周辺メッシュ数から除くために使用。詳細仕様は [8.2.1](#821-n03-前処理済みファイル詳細仕様) 参照 |
...
**説明**:
  ...
  - 同一ピーク座標（ズームレベル15 のピクセル座標（タイル座標とタイル内の画素位置の組）が一致）のレコードを同一ピークとして重複排除し、統合する。広域モードの per-mesh CSV もピーク座標をズームレベル15 座標で出力するため（[FR-014](#fr-014-広域結合解析オーケストレーション) 参照）、本判定は通常・広域の区別なく機能する
    - **代表採用ロジック**（重複した複数解析レコードから 1 件を選ぶ優先順位。決定論的に一意となるよう以下の順で適用する）:
      ...
      4. 全レコードが `key_col_resolved=false` の場合は `key_col_resolved=false` のまま維持する。代表は通常モード（接頭辞 `3`）を広域モードより優先し、さらに `analysis_id` 昇順で一意に決定する（プロミネンス未確定のため、[FR-022](#fr-022-コル充足判定) がエスカレーション対象として報告する）
    ...
    - `analysis_count`: 重複排除前の通常モード行（`analysis_id` 接頭辞 `3`）の出現回数（実際の通常解析回数）。広域モード行は数えない（`expected_count` と同じ母集団で比較するため）
    - `expected_count`: **日本全土1次メッシュコードリストから北方領土除外メッシュを除いた集合を基準**に算出する期待解析回数（`対象1次メッシュコードリスト` で範囲を絞っても本値は変わらない）。通常モード行（`analysis_id` 接頭辞 `3`）のみを対象とし、3×3 隣接カウントで算出する（ピークの所属 1次メッシュコードを基準に、日本全土 1次メッシュコードリスト内に存在し北方領土除外メッシュに該当しない周辺 1次メッシュ数をカウント。コーナーメッシュ=4・エッジメッシュ=6・中央メッシュ=9 が期待値。周辺メッシュでも存在しない、または北方領土除外メッシュに該当する場合はその分だけ減る）。広域モード行（接頭辞 `4/5/6`）は `expected_count` の算出対象外（空欄）。本リストは常に存在するため、通常モード行で空欄になることはない
  ...
  - プロミネンス最終フィルタ: **プロミネンス最終フィルタ閾値**（[データ辞書参照](#221-設定可能項目)）以上（[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) の一次フィルタ通過済みのレコードに適用）。代表採用後に `key_col_resolved=false`（プロミネンス未確定）のレコードは適用対象外とし、そのまま出力する（[FR-007](#fr-007-per-mesh-csv-出力プロミネンス閾値適用) の一次フィルタと同じ扱い。[FR-022](#fr-022-コル充足判定) のエスカレーション対象を残すため）
  - **広域確定ピークの除外一覧**: 代表レコードが広域モード（`analysis_id` 接頭辞 `4`/`5`/`6`）で `key_col_resolved=true`、かつプロミネンスが **プロミネンス一次フィルタ閾値** 以上・**プロミネンス最終フィルタ閾値** 未満のため最終フィルタで除外したピークを、警告ログに一覧で出力する（ピーク座標・ピーク標高・コル標高・プロミネンス・`analysis_id`）。広域解析の max pooling によるプロミネンス過小評価で落ちた可能性を利用者が確認するためで、`merged_peak.csv` の内容は変えない（[ADR-SRS-056](decisions/ADR-SRS-056-wide-analysis-prominence-underestimate-warning.md)）
...
| データ名 | 種別 | 形式 | 備考 |
|---|---|---|---|
| コル未確定ピーク座標リスト（`key_col_unresolved_peaks-<N>.csv`） | 内部データ | CSV | ファイル: `$DATA_DIR/results/key_col_unresolved_peaks-<N>.csv`（[8.1 参照](#81-内部データ一覧)）。N = 生成段階タグ（3×3 後は `-3`、4×4 後は `-4`、5×5 後は `-5`、6×6 後は `-6`）。カラム: `peak_lat, peak_lon`（ヘッダー行あり・1行1件）。[FR-023](#fr-023-解析パイプライン制御) の制御に使用。未確定ピークがゼロの場合は生成しない |
...
- **対応 UR**: [UR-003](10_URD.md#ur-003), [UR-005](10_URD.md#ur-005), [UR-006](10_URD.md#ur-006)
...
| 標高タイル（ローカルキャッシュ） | 内部データ | 必須 | — | [8.1 参照](#81-内部データ一覧)。ファイルの mtime だけを `gsi_tile_latest_date` の算出に使う（標高値は読まない） |
...
**説明**:
  ...
  - **仮サミットコード割り当て**（`new` および `dominant` ピーク）:
    ...
    - **採番タイミング**: 本 FR の実行ごとに、入力の `merged_peak.csv` から全件再採番する。同一の `merged_peak.csv`・SOTA サミットリスト CSV・N03 前処理済み地域 GeoJSON からは常に同一の仮サミットコードが得られる（決定論性は [NFR-003](#nfr-003-再現性決定論的出力) が保証する）。
  ...
  - **rationale プロパティ生成**（各フィーチャの `rationale` プロパティに格納する申請書根拠テキスト。HTML ビューアで編集可能・[FR-011](#fr-011-申請書-xlsx-生成) の XLSX 列 I に転記）:
    ...
    - **※4 削除根拠フォーマット**（バッチ生成時の category=delete の既存 SOTA サミット Point に付与。全 review は空文字）:
      ...
      ```text
      国土地理院標高タイルを解析し、{親の参照}に従属している事を確認
      ```

      `{親の参照}` は、主ピークが matched なら `{dominant_peak_code}`（正式コード）、dominant なら `追加申請するピーク（{peak_lat},{peak_lon}）`（主ピークの座標。※2 と同じ表記）とする。仮サミットコードは申請書に出力しないため（[FR-011](#fr-011-申請書-xlsx-生成) ※6）、座標で追加行と突き合わせられるようにする
...
| `summit_name` | サミット名（matched のみ・英語/ローマ字。new / dominant・ambiguous は空文字。new / dominant の名称は [FR-019](#fr-019-html-ビューア機能仕様) で利用者が入力する） |
| `summit_name_jp` | 日本語山岳名（matched のみ・本 FR が geojson_v{N} から取得し格納。未取得時・new / dominant・ambiguous は空文字。new / dominant の名称は [FR-019](#fr-019-html-ビューア機能仕様) で利用者が入力する。[ADR-SRS-036](decisions/ADR-SRS-036-new-peak-name-input-requirement.md)） |
| `peak_elev` | 検出標高（m） |
...
**説明**:
  ...
  - **山岳名入力 UI**:
    - `category=add` のピーク（`match_status` が `new` または `dominant`）: クリックで開くポップアップまたはサイドパネルに「山岳名JP」（必須）「山岳名EN」（任意）入力フィールドを表示する。今回の申請対象に山岳名JP が未入力のピークが存在する状態で申請書（[FR-011](#fr-011-申請書-xlsx-生成)）をエクスポートする場合は、該当ピーク一覧を警告表示する。ただし公式申請の最終判断は SOTA 日本支部担当者が行うため、エクスポートはブロックせず空欄のままの続行も許容する（ソフト必須。[ADR-SRS-036](decisions/ADR-SRS-036-new-peak-name-input-requirement.md)）
...
| ライブラリ（SheetJS） | 外部I/F | 必須 | — | CDN からブラウザが実行時取得（[§9](#9-外部システム依存関係環境)）。取得失敗時の扱いは本 FR の異常系 |
...
- **概要**: `merged_summit.geojson`（[FR-009](#fr-009-sotaリスト突合match_status-判定) 出力の中心データ）から、[FR-019](#fr-019-html-ビューア機能仕様) でユーザーが編集した山岳名（`summit_name_jp`・`summit_name`）を反映したサミット一覧（申請内容反映版）を **HTML ビューア（[FR-019](#fr-019-html-ビューア機能仕様)）内でブラウザ生成**する。生成した XLSX は申請エビデンス ZIP（[FR-021](#fr-021-申請エビデンス-zip-生成)）に同梱してダウンロードする（[UR-005](10_URD.md#ur-005) 対応）。担当者指定削除の category・review_decision・review_note と、申請除外の application_exclusion・exclusion_note も反映する。rationale 列は一覧に持たず、根拠全文は申請書列 I と ZIP 内 GeoJSON に反映する。
...
|---|---|---|---|---|
| 突合済み統合 GeoJSON（`merged_summit.geojson`） | 外部I/F | 必須 | — | 作業用ビューアデータファイルとして読み込み済み（[FR-013](#fr-013-html-ビューア生成) 出力） |
| ビューア上の表示・編集状態 | 内部データ | 必須 | — | [FR-019](#fr-019-html-ビューア機能仕様) 出力。エクスポート時点のスナップショット（localStorage を直接読むのではなく、[FR-019](#fr-019-html-ビューア機能仕様) の引き継ぎ確認を経た現在の編集状態）。山岳名・rationale を未編集の対象は初期値（[FR-009](#fr-009-sotaリスト突合match_status-判定) 自動生成テンプレート）のまま |
| ライブラリ（JSZip・SheetJS） | 外部I/F | 必須 | — | CDN からブラウザが実行時取得（[§9](#9-外部システム依存関係環境)）。取得失敗時の扱いは本 FR の異常系 |
...
- **プロミネンス最終フィルタ閾値**（[データ辞書参照](#221-設定可能項目)）以上のピークを出力すること（代表採用後に `key_col_resolved=false` のピークは最終フィルタの対象外。[FR-008](#fr-008-per-mesh-csv-統合) 参照）
...
- 広域解析（[FR-014](#fr-014-広域結合解析オーケストレーション)）で確定したプロミネンスは、2×2 max pooling によりコル標高が高めに評価されるため、実際より小さく出うる（既知の限界）。最終フィルタ閾値近傍で除外された該当ピークは [FR-008](#fr-008-per-mesh-csv-統合) が警告ログで一覧化する（[ADR-SRS-056](decisions/ADR-SRS-056-wide-analysis-prominence-underestimate-warning.md)）
...
- 全国処理範囲は [UR-001](10_URD.md#ur-001) の対象集合（採用時175メッシュ）とする。
  期待メッシュ集合を出典・URD から製品の入力リストとは独立に導き、①製品対象リスト、②正常完了したコード集合と規則による省略集合（北方領土除外メッシュ）の和、とそれぞれ照合する。
  件数一致だけで合格にせず、重複・欠落、各メッシュの当該実行ログと成果物を確認する。
  正常完了集合と省略集合が重ならないこと、製品が実行ログに残す省略集合が独立導出した省略集合と一致することも確認する。
  竹島のメッシュ単位除外と、北方領土のタイル単位除外および除外メッシュの省略を区別し（北方領土除外メッシュは期待集合から削除しない。詳細: [ADR-SRS-053](decisions/ADR-SRS-053-northern-territories-tile-intersection-and-excluded-meshes.md)）、欠損入力や異常終了で対象集合を減らさない。
  省略集合は期待メッシュ集合と同じく製品出力から独立に導き、N03 の版を製品が使用した版と同一に固定したうえで事前固定する。
...
- 広域解析（[FR-014](#fr-014-広域結合解析オーケストレーション)、N≤6・ズームレベル14）の解析範囲は、画素数で通常解析（3×3・ズームレベル15）と同等である。広域解析のメモリ実測は HLD 以降で行う
...
- 同一ローカルキャッシュ・同一パラメータで実行した場合、バッチ出力（CSV・GeoJSON・XLSX）が同一の内容になること（ブラウザ出力は上記の編集状態・台帳も入力条件に含める）（`metadata.generated_at` 等の実行時タイムスタンプおよび `metadata.software_version` を除く。ピーク座標・標高・突合結果・採番が同一であることを保証する。SOTA サミットリストの有効判定は CSV の基準日で行い、実行日に依存しない（[ADR-SRS-054](decisions/ADR-SRS-054-summitslist-validto-reference-date.md)））（タイル再取得・キャッシュ削除によりローカルキャッシュの内容が変わった場合は再現性対象外）
...
- **タイル取得のリクエスト間隔**（[データ辞書参照](#221-設定可能項目)）を守ってリクエストする
- サーバーがリクエスト過多・一時利用不可を示す応答を返した場合は、待機時間を段階的に延ばしながらリトライする（該当する応答の具体値は HLD で定義する）

### NFR-005: 処理時間目標

- **対応 UR**: [UR-001](10_URD.md#ur-001)
- タイル取得: 処理範囲175メッシュで12時間以内（約4分/メッシュ）を目標とする（ハードリミットではなく目安値。未達でも異常終了しない）
- 解析: 処理範囲175メッシュで24時間以内（約8分/メッシュ）を目標とする（ハードリミットではなく目安値。未達でも異常終了しない）
...
文書不備・機能不備と区別する。[SRS §2.2.1](#221-設定可能項目) に定める見直し（要確認サミット件数しきい値の実データによる見直し、delete判定ゾーン比高上限の ADR 判断による変更）に従い、
...
| 11 | 地理院基準点タイル | 入力 | GeoJSON（ベクトルタイル） | [FR-019](#fr-019-html-ビューア機能仕様) / [FR-020](#fr-020-公開用ビューア配信) | 基準点レイヤー ON 時にブラウザから実行時取得。出典・利用形態: [SOURCES.md](../ref/SOURCES.md) |
...
申請書 XLSX（[6.2.1](#621-申請書-xlsx)）と本 ZIP は別の操作で、それぞれの操作時点の表示・編集状態から生成する。提出する一組は、編集を終えた同じ状態から続けて出力する（利用者手順として [NFR-011](#nfr-011-手順文書による環境構築成果物生成の完遂性) の手順文書に記載する）。
...
| 4 | 日本全土1次メッシュコードリスト | `params/` 直下（ファイル名は HLD で定義） | — | [FR-001](#fr-001-標高タイル事前取得) / [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) / [FR-008](#fr-008-per-mesh-csv-統合) / [FR-014](#fr-014-広域結合解析オーケストレーション) / [FR-017](#fr-017-n03-行政区域前処理データ準備) / [FR-018](#fr-018-per-mesh-ピーク候補-geojson-統合) | [日本の国土にかかる第1次地域区画](../ref/SOURCES.md#日本の国土にかかる第1次地域区画)を参照し、ベースラインとして本システムで用意する。[FR-008](#fr-008-per-mesh-csv-統合) では北方領土除外メッシュを除いた集合を `expected_count` 算出基準として使用する |
| 5 | 標高タイル（ローカルキャッシュ） | `$DATA_DIR/tiles/{サービス名}/{z}/{x}/{y}.png`<br>サービス名: DEM5a=`dem5a_png` / DEM5b=`dem5b_png` / DEM5c=`dem5c_png` / DEM10b=`dem_png` | [FR-001](#fr-001-標高タイル事前取得) | [FR-004](#fr-004-33メッシュ結合解析オーケストレーション) / [FR-014](#fr-014-広域結合解析オーケストレーション) / [FR-009](#fr-009-sotaリスト突合match_status-判定)（mtime のみ） | |
...
| 省略時の動作 | [FR-009](#fr-009-sotaリスト突合match_status-判定) がエリアコード付与をスキップ（`ZZ/ZZ-A00` 形式）し、警告ログを出力する |
...
| 生成方法 | [FR-017](#fr-017-n03-行政区域前処理データ準備) で生成 |
| 省略時の動作 | [FR-009](#fr-009-sotaリスト突合match_status-判定) が市区町村名を空文字付与して続行し、警告ログを出力する |
...
- 竹島（島根県）に所在するピークは SOTA 日本支部の管轄外（韓国 SOTA サミット HL/GB-430 として登録済み）のため解析対象外とする。具体的には日本全土1次メッシュコードリスト（[8.1 参照](#81-内部データ一覧)）から竹島が含まれるメッシュ 5531 を除くことで除外する（根拠: [ADR-URD-009](decisions/ADR-URD-009-takeshima-exclusion.md)）
...
- 日本以外の SOTA 申請
- 北方領土に所在するピーク（SOTA 日本支部の管轄外のため。除外方法は [FR-001](#fr-001-標高タイル事前取得)・[FR-017](#fr-017-n03-行政区域前処理データ準備) で規定。陸地が北方領土のみとなる1次メッシュは省略され、175メッシュの期待集合からは削除しない）
- 竹島に所在するピーク（韓国 SOTA サミット HL/GB-430 として登録済み。SOTA 日本支部の管轄外のため。除外方法は日本全土1次メッシュコードリスト（[8.1 参照](#81-内部データ一覧)）からメッシュ 5531 を除く。根拠: [`ADR-URD-009`](decisions/ADR-URD-009-takeshima-exclusion.md)）
- 新規サミットの山岳名取得（解析結果から自動取得する手段がなく技術的に困難なため対象外。HTML ビューアで OSM・国土地理院地図を参照しながら人間系で確認・記入すること）。既存サミットの日本語山岳名（`summit_name_jp`）は SOTA 既存サミット GeoJSON（geojson_v{N}。[7.2.5 参照](#725-sota-既存サミット-geojsongeojson_vn)）から自動取得する
