...
**説明**:
  ...
  - 申請出力は `category` を正とし add / delete / band_change のみを対象とする。`match_status=delete` でも `category=review` の行は出力しない。複数登録の保留組から追加・変更・削除行を生成せず、組数・保留登録件数を警告して無関係な候補の出力を続行する（[FR-019](#fr-019-html-ビューア機能仕様)）。
...
- **対応 UR**: [UR-005](10_URD.md#ur-005), [UR-006](10_URD.md#ur-006)（公開用データの供給）, [UR-011](10_URD.md#ur-011)
...
| 提供機能 | [FR-019](#fr-019-html-ビューア機能仕様) の閲覧系機能のうち編集 UI・エクスポート・localStorage を除く全て（「全データ」popup を含む）。詳細は [FR-020](#fr-020-公開用ビューア配信) 参照 |
