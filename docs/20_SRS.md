...
**説明**:
  ...
  - **解析対象メッシュの決定**: 対象メッシュを中心とした 3×3 のメッシュグリッド（最大 9 メッシュ）のうち、1次メッシュコードリスト（省略時は日本全土1次メッシュコードリスト）に存在し、かつ北方領土除外メッシュリストに含まれないものを解析対象メッシュとする（除外メッシュはリスト外メッシュと同じ扱いとなり、解析対象メッシュ範囲情報・解析範囲の bounding box・解析済みウィンドウメッシュ集合からも外れる。詳細: [ADR-SRS-053](decisions/ADR-SRS-053-northern-territories-tile-intersection-and-excluded-meshes.md)）。
