...
| 公開用ビューア | 静的ホスティング上で公開用データ（申請エビデンス GeoJSON）を閲覧専用表示 | 公開用データ（申請エビデンス GeoJSON）・テンプレート | 公開用ビューア画面 |
...
可視化生成コンポーネント   HTML ビューア生成
       └─ merged_viewer.html        ← 編集可能なローカル HTML ビューア（テンプレート + データファイルの 2 ファイル構成・rationale 編集機能付き）
...
**説明**:
  ...
  - 公開用は閲覧専用（編集 UI なし・XLSX エクスポートなし・localStorage 不使用）。ZIP 同梱の申請内容反映版 XLSX は公開しない
  ...
  - **異常系**: 公開用データの一部が取得できない場合も残りを表示し、欠落を利用者が判別できること。5 本の `metadata.generated_at` が一致しない場合は不整合として利用者に提示すること（表示の詳細は HLD に委ねる）。ホスティングの障害は外部境界（[§9](#9-外部システム依存関係環境)）
...
| 用途 | 申請先への証跡共有等の外部公開。リリース版と開発版を同時に配信でき、閲覧者がどちらかを判別できる（判別手段は HLD）。ZIP 同梱の申請内容反映版 XLSX は公開しない |
| URL 構成 | HLD で定義 |
...
| 提供方法 | 申請に添える ZIP から取り出し、リポジトリの公開用データ置き場に配置する（反映手順とリリース版・開発版の分け方は HLD・運用手順） |
