...
通常候補の申請除外を別経路として追加した。担当者指定削除は除外操作の対象外であり、申請から外すには本 ADR の「要確認に戻す」を使う。除外台帳は review_decision を復元・除外せず、通常名称・rationale・手動削除の採用履歴を交換しない。

新世代で個別再採用を要する本 ADR は削除申請を増やす判断である。申請を減らす除外側は同一対象・判断時条件の一致と継承/読込への同意を前提に再適用する。両判断で条件を分け、除外による申請漏れのリスクは適用/未適用一覧と件数で提示する。
