...
| 用語（本書での表記） | 正式名称 | 説明 |
|---|---|---|
| メッシュ | 第一次地域メッシュ（第一次地域区画） | 国土地理院が定める地域メッシュのうち、約80km四方の区画。4桁の数値コードで識別される（例: 5339）。本プロジェクトの解析単位。URD/SRS の「1次メッシュ」も同義で、「land 1次メッシュ」は陸地を含む1次メッシュを指す。 |
...
| 独立峰 | — | 周囲に同等以上の山がなく、Keyコルが3×3メッシュの解析範囲の外に出るピーク（例: 大山）。通常解析ではコルが確定せず、広域結合解析（[FR-014](20_SRS.md#fr-014-広域結合解析オーケストレーション)）で扱う（[UR-007](10_URD.md#ur-007)）。 |
...
| 用語（本書での表記） | 正式名称 | 説明 |
|---|---|---|
| 基準ピークセット | — | 固定した入力 DEM と仕様の下で適格と独立確認し、試験前に内容を固定した代表ピークの集合。必須レコードと、根拠・許容位置・代表区分・版を持つ。SOTA 全登録や製品出力をそのまま正解にした集合ではない。製品入力ではなく試験用（[NFR-001](20_SRS.md#nfr-001-精度プロミネンス判定)、[ST §3.8.2](70_ST.md#382-検出網羅性の基準セットと判定手順)） |
| 基準セット再現率 | — | 事前固定した全必須レコードのうち、全国最終統合出力へ一意に照合でき、コル確定かつ最終閾値以上を満たした割合。合格は検出漏れ0件・100%。空セットは合格にせず、未知の全ピークに対する漏れゼロの保証とも区別する |
...
| 用語（本書での表記） | 正式名称 | 説明 |
|---|---|---|
| サミット候補 | — | SOTA サミットの要件（プロミネンス ≥ 150m）を満たすとして検出・抽出の対象となるピークの総称。既存サミットに対応するピークも含む。新規追加の申請対象は[申請カテゴリ](#申請カテゴリ) `add`（`peak.match_status` ∈ {new, dominant}）で表す。 |
